...
- 综合判断区必须位于页面高优先级区域，负责给出“当前最值得参考的策略结论”。
- 页面必须减少重复 badge、重复摘要和重复证据，避免同一语义在多个区域重复堆叠。
- 页面必须压缩低优先级原始细节，把关键结论、风险、触发条件和失效条件优先呈现。
- 原始结构详情、调试字段和推导证据可以保留，但应降级为支撑信息或 debug 信息，不能继续淹没主结论。
...
AI 最终输出中的“操作策略参考”应作为页面综合判断区的首要参考来源，但必须满足：

- AI 只能在后端硬门控允许的范围内做综合表达。
- AI 可以整合多周期、证据链和风险语境来组织语言。
- AI 不得突破 `deterministic_decision` 已明确给出的等待、规避、降级仓位和风险控制约束。
...
起点不是结构资格系统里的一个附属细节，而是后续结构判定、节点编号、执行锚点和页面解释链的前置基础层。
...
- 明确三类起点的职责分层：
  - `background_origin` = 宏观参考原点
  - `focus_origin` = 当前结构解释起点
  - `execution_origin` = 当前交易执行起点
- 锁定 `focus_origin` 的稳定优先级：
  - 主峰 / 主谷优先：`peak_extreme` / `valley_extreme`
  - 其次最近成熟组件边界：`recent_component`
  - 最后退回：`macro_origin`
- 锁定窗口外原点规则：宏观原点落在当前窗口外时，禁止偷换成窗口首个可见点。
- 锁定“全局结构”和“聚焦结构”的语义分层：
  - `raw_classification` 表示宏观 / 全局结构
  - `focus_classification` 表示聚焦后结构
  - `trinity_decision.structure.type` 表示用于交易决策的聚焦结构
- 锁定编号规则：
  - 标准结构从当前结构起点重新编号
  - 延伸 / 通道 / 大平台 / 未完成 / 复杂结构停止标准编号，只保留可解释锚点
- 为页面与 AI 契约提供稳定来源说明，避免后续结构资格和交易资格阶段反复返工。
...
- 增加边界字段和边界交易标记。
- 这一阶段建立在 `Phase 2.0` 已锁定的起点语义之上，不再重新发明起点优先级和编号规则。
...
- 页面按“综合判断 -> 关键规则链 -> 支撑细节”的信息层级重构，解决当前重复、冗余、信息过载导致关键信息被淹没的问题。
- 页面新增综合判断区域，优先展示 AI 最终分析形成的操作策略参考，同时保留后端确定性结论作为硬边界与证据来源。
- 页面减少重复信息堆叠，把原始详情降级为支撑信息或 debug 数据，而不再作为主阅读路径。
- AI 收到确定性结论和证据，并输出可直接用于页面综合判断区的最终分析摘要。
- 原始详情保留为 debug 数据，但不再作为主契约。
- 页面交互、布局方式和交互细节在正式修改前，必须先与用户确认后再实施；Phase 4 不得默认直接推进未确认的交互改造。
...
```bash
curl -s "http://localhost:5001/api/stock/analysis?code=300274&levels=weekly"
```

同时建议追加一次全级别回归，确认级别矩阵和起点字段没有在聚合响应里丢失：

```bash
curl -s "http://localhost:5001/api/stock/analysis?code=300274&levels=weekly,daily,hour60,hour30,hour15"
```

默认开发端口使用 `5001`；若本机 `5001` 被占用，可以切换到其他空闲端口，但必须明确说明实际使用端口。
...
- 级别嵌套权限可以降级子级别交易信号。
- 页面可以不做推断地展示整条规则链。
- 页面必须有一个高优先级综合判断区域，能够清楚呈现最值得参考的当前策略结论。
- 页面不得出现明显的信息重复、证据重复或原始字段堆叠导致主结论被淹没的情况。
- AI payload 包含确定性结论、交易资格、触发条件、失效条件和风险标记。
- 页面中的操作策略参考以 AI 最终分析为准，但 AI 结论不得突破后端确定性硬门控。
